...
import { runAgent } from '~/lib/openclawService';
...
export async function streamText(props: {
  ...
  if (currentProvider === 'OpenClaw') {
    ...
    const openClawStream = new ReadableStream({
      async start(controller) {
        try {
          ...
          logger.error('OpenClaw failed, attempting Llama fallback:', error);
          // FALLBACK TO LLAMA (GROQ)
          try {
            const fallbackProvider = PROVIDER_LIST.find((p) => p.name === 'Groq') || PROVIDER_LIST.find((p) => p.name === 'Ollama');
            if (fallbackProvider) {
              const fallbackModel = fallbackProvider.name === 'Groq' ? 'llama-3.3-70b-versatile' : 'llama3.2:latest';
              logger.info(`Using fallback: ${fallbackProvider.name} with model ${fallbackModel}`);
              
              const fallbackParams = {
                model: fallbackProvider.getModelInstance({
                  model: fallbackModel,
                  serverEnv,
                  apiKeys,
                  providerSettings,
              ...
              const result = await _streamText(fallbackParams);
              ...
              controller.error(new Error('OpenClaw failed and no fallback provider (Groq/Ollama) found.'));
            }
          } catch (fallbackError) {
    ...
    return {
      fullStream: openClawStream,
      toDataStream: () => openClawStream,
      mergeIntoDataStream: (dataStream) => {
        ...
        reader.read().then(function processText({ done, value }) {
          ...
          dataStream.enqueue(encoder.encode(value));
